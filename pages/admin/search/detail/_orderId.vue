  ...
  <div class="container py-5">
    ...
    <div class="row border-md border-top py-2 mb-3 mx-1 mx-sm-0">
      ...
      <div class="col-12">
        <div class="row mb-2">
          <div class="col-4 col-md-2">
            <label class="mb-0">訂購者備註</label>
          </div>
          <div class="col-8 col-md-10">
            <EditTextArea
              :data-value.sync="orderInfo.buyerRemark"
              :is-edit="orderInfo.isEdit"
              placeholder="輸入備註內容"
            />
          </div>
...
export default {
  ...
  methods: {
    ...
    setDiscountPrice(orderQty, index) {
      ...
      const isFromGroup = this.orderInfo.isFromGroup

      if (isFromGroup) {
        // 過渡 - 配合舊資料
        if (!orderItem.promote) {
          orderItem.promote = 3500
        }
        const groupPrice = orderItem.promote
        orderList[index].unitPrice = groupPrice
        orderList[index].totalPrice = orderList[index].qty * groupPrice
      } else {
        // 過渡 - 配合舊資料
        if (!orderItem.discountList) {
          orderItem.discountList = {
            '>=100': 4200,
            '<100': 4500,
            '<50': 4900,
            '<20': 5200,
        ...
        const priceList = orderItem.discountList
